...
class BasketsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create basket" do
    assert_difference('Basket.count') do
      post baskets_url, params: { basket: { name: @basket.name, status: @basket.status, user_id: @basket.user_id } }
    end
  ...
  test "should update basket" do
    patch basket_url(@basket), params: { basket: { name: @basket.name, status: @basket.status, user_id: @basket.user_id } }
